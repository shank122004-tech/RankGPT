...
(function () {
  ...
  // Detect very low-end devices (< 2GB RAM hint or very small screen)
  ...
  const ORB = isLowEnd ? 90 : isMobile ? 110 : 140;
  const WRAP = isLowEnd ? 160 : isMobile ? 200 : 260;
  ...
  overlay.innerHTML = `
    ...
    <div id="iCenter">
      ...
      <div id="iMadeInIndia">🇮🇳 <span>Proudly Made in India</span></div>
    </div>
  ...
  style.textContent = `
    ...
    #iMadeInIndia{margin-top:14px;display:flex;align-items:center;gap:6px;font-family:'Rajdhani',sans-serif;font-size:${isLowEnd?'10px':isMobile?'11px':'12px'};font-weight:700;letter-spacing:0.18em;text-transform:uppercase;background:linear-gradient(90deg,#FF9933 0%,#ffffff 50%,#138808 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;opacity:0;will-change:opacity;animation:iMIIPulse 2.5s ease-in-out infinite}
    @keyframes iMIIPulse{0%,100%{filter:brightness(1)}50%{filter:brightness(1.35)}}
  ...
  function initBg() {
    ...
    const CNT = isLowEnd ? 40 : isMobile ? 80 : 220;
    ...
    const LINES = isLowEnd ? 20 : isMobile ? 45 : 110;
    ...
    const NODE = isLowEnd ? 4 : isMobile ? 10 : 26;
    const nodes = [];
  ...
  function fireParticleBurst(cb) {
    ...
    const BURST = isLowEnd ? 35 : isMobile ? 70 : 150;
  ...
  function startIntro() {
    ...
    const mii      = document.getElementById('iMadeInIndia');
    ...
    // Made in India badge appears with tagline
    tl.to(mii, {opacity:1, y:0, duration:.7, ease:'power2.out'}, 2.4);
    ...
    tl.to([tag,stats,prog,statusW,mii], {opacity:0,y:-20,duration:.32,stagger:.04}, 4.6);
