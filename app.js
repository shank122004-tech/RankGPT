...
'https://geminivision-56khnynjia-uc.a.run.app';

// Gemini direct API — use Firebase Function proxy to avoid CORS/403 issues
// The /api/gemini function in index.js proxies to Gemini with server-side key
const GEMINI_DIRECT_API_KEY = 'AIzaSyCmzArFqO2Y1-Mm4THkiN7y_1xjogWNqyY';
const GEMINI_DIRECT_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent';
// Firebase hosted function proxy (avoids 403 on direct key)
...
const CASHFREE_ORDER_URL = 'https://createcashfreeorder-419137308157.us-central1.run.app';
const VERIFY_PAYMENT_URL = 'https://verifypayment-419137308157.us-central1.run.app';
...
- Tease them: "teri girlfriend se zyada notes likhti hai toh usse padha le 😂"
...
function getSystemPrompt() {
  ...
  const langMap = {
    hinglish: 'Always respond in Hinglish (mix of Hindi and English naturally). Example: "Achha beta, yeh formula yaad karo!"',
    hindi: 'Always respond in simple Hindi (Romanized script is fine for math terms).',
  ...
  const wordLimit = state.shortResponseMode ? 90 : (grade && grade <= 2 ? 100 : grade && grade <= 5 ? 140 : grade && grade <= 8 ? 200 : 290);
  ...
  if (isClassMode) {
    ...
    if (grade && grade <= 2) {
      ...
      teacherPersona = 'You are a warm, nurturing primary school teacher talking to a 6-7 year old child.';
      languageRule = 'Use ONLY very short, simple words (1-2 syllable words). Write in short sentences of 5-8 words maximum. NEVER use difficult vocabulary. Use lots of 🌟⭐🎉🐶🍎 emojis to make it fun and engaging.';
...
async function callAI(userMessage, chatHistory = [], imageBase64Array = [], pdfBase64 = null) {
  ...
  if (hasImage || hasPdf) {
    ...
    // Step 1: Use Gemini to extract/parse the visual content into text
    const visualText = await callGeminiVision(userMessage, chatHistory, imageBase64Array, pdfBase64);
    // Step 2: For vision-pro/pro models, re-analyze the extracted text with DeepSeek for deeper insight
...
Now give an enhanced, detailed explanation with step-by-step reasoning for: ${userMessage}`,
        ...
        return visualText; // fallback to gemini result
      }
    }
    return visualText;
  } else {
...
async function callGeminiVision(userMessage, chatHistory = [], imageBase64Array = [], pdfBase64 = null) {
  ...
  const systemPrompt = getSystemPrompt();
  userParts.push({ text: (systemPrompt ? systemPrompt + '\n\n' : '') + userMessage });
  ...
  // Try Firebase Function proxy first (avoids CORS/403 on direct key)
  try {
    const proxyBody = { ...body, mode: state.sscMode, lang: state.aiLang, shortMode: state.shortResponseMode };
    ...
    console.warn('[Gemini] Proxy failed, trying direct:', proxyErr.message);
  }

  // Direct Gemini API fallback — uses gemini-1.5-flash (no billing restrictions)
  const directRes = await fetch(`${GEMINI_DIRECT_URL}?key=${GEMINI_DIRECT_API_KEY}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!directRes.ok) {
    ...
    throw new Error(`Gemini Error ${directRes.status}: ${errData?.error?.message || 'Check API key or billing'}`);
  }
  const directData = await directRes.json();
  const text = directData.candidates?.[0]?.content?.parts?.[0]?.text;
  return text || 'Sorry, could not analyze this content. Please try again.';
}
  ...
  modal.innerHTML = `
    <div style="background:linear-gradient(135deg,#0f0c1f,#1a1435);border:1px solid rgba(108,99,255,0.35);border-radius:20px;padding:28px 24px;max-width:360px;width:100%;text-align:center;box-shadow:0 0 60px rgba(108,99,255,0.15);">
      ...
      <ul style="text-align:left;font-size:12px;color:rgba(200,195,255,0.8);list-style:none;padding:0;margin:0 0 20px 0;display:flex;flex-direction:column;gap:6px;">
        ${isVision
          ? '<li>✅ Gemini Vision + DeepSeek dual-AI</li><li>✅ Handwritten notes recognition</li><li>✅ PDF chapter-wise solving</li><li>✅ Diagram & graph analysis</li>'
...
window.selectPersona = function(persona) {
  ...
  const names = { bhaiya:'Bhaiya', didi:'Didi', teacher:'Teacher', friend:'Best Friend', professor:'Professor', mentor:'Mentor', motivator:'Motivator', coach:'Coach' };
  showToast(`✅ ${names[persona] || persona} mode activated! I'll talk to you like your ${names[persona] || persona}! 🎉`);
  _doConfetti();
};
...
(function initChatDictation() {
  const GOOGLE_STT_KEY = typeof GEMINI_DIRECT_API_KEY !== 'undefined' ? GEMINI_DIRECT_API_KEY : '';
  ...
  function startDictation() {
    ...
    recognition.onend = function() {
      stopDictation();
      if (textarea) {
        textarea.placeholder = 'Ask your question… or tap 🎤 to speak';
        textarea.focus();
      }
    };
...
(function() {
  ...
  const GOOGLE_TTS_KEY = typeof GEMINI_DIRECT_API_KEY !== 'undefined' ? GEMINI_DIRECT_API_KEY : '';
  ...
  window.speakMessage = async function(btn) {
    ...
    // Use Google TTS if teacher/voice model selected, or if user is premium
    const useGoogleTTS = (typeof selectedModel !== 'undefined' && (selectedModel === 'teacher' || selectedModel === 'voice'))
      || (typeof state !== 'undefined' && state.isPremium);

    try {
      if (useGoogleTTS && GOOGLE_TTS_KEY) {
        try {
          const audioSrc = await speakWithGoogleTTS(text, lang);
          const audio = new Audio(audioSrc);
          currentAudio = audio;
          audio.onended = () => { btn.classList.remove('speaking'); btn.textContent = '🔊'; currentAudio = null; };
          audio.onerror = () => { btn.classList.remove('speaking'); btn.textContent = '🔊'; currentAudio = null; };
          await audio.play();
          return;
        } catch(e) {
          console.warn('[speakMessage] Google TTS failed, using browser TTS:', e.message);
        }
      }
