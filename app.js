...
// ===== CONFIGURATION =====
const BACKEND_URL = "https://api-56khnynjia-uc.a.run.app";

const GEMINI_API_URL = `${BACKEND_URL}/gemini`;
const DEEPSEEK_API_URL = `${BACKEND_URL}/deepseek`;
...
async function handleImageSelect(e) {
  ...
  if (!canSendImage()) { showToast(`❌ Daily image limit reached. Upgrade to Premium!`); openPremiumModal(); return; }
...
async function handlePdfSelect(e) {
  ...
  if (!canSendPdf()) { showToast(`❌ Daily PDF limit reached. Upgrade to Premium!`); openPremiumModal(); return; }
...
async function sendMessage() {
  ...
  if (isSending) return;
  if (hasImages && !canSendImage()) { showToast(`❌ Daily image limit reached. Upgrade to Premium!`); openPremiumModal(); return; }
  if (hasPdf && !canSendPdf()) { showToast(`❌ Daily PDF limit reached. Upgrade to Premium!`); openPremiumModal(); return; }
  if (!canSendText()) { showToast(`❌ Daily text limit (${FREE_TEXT_LIMIT}) reached. Upgrade!`); openPremiumModal(); return; }
  isSending = true;
...
function setupVoiceInput() {
  if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) { if (dom.voiceInputBtn) dom.voiceInputBtn.style.display = 'none'; return; }
  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  const recognition = new SR();
  recognition.lang = 'hi-IN'; recognition.interimResults = false; recognition.continuous = false;
  dom.voiceInputBtn?.addEventListener('click', () => { dom.voiceInputBtn.classList.add('recording'); showToast('🎤 Listening... Speak now', 3000); recognition.start(); });
  recognition.onresult = (e) => { const transcript = e.results[0][0].transcript; dom.messageInput.value = transcript; dom.messageInput.dispatchEvent(new Event('input')); showToast(`🎤 "${transcript}"`, 2000); setTimeout(() => { if (transcript.trim()) sendMessage(); }, 500); };
...
// ===== PREMIUM =====
function openPremiumModal() { renderPremiumModal(); dom.premiumModal.classList.add('active'); }
...
async function handlePayment(planId) {
  ...
  const orderId = `sscprepai_${planId}_${state.user.uid}_${Date.now()}`;

  // Store pending order info for post-payment verification
  ...
  const paymentUrl = `https://payments.cashfree.com/forms/sscprepai?amount=${plan.price}&order_id=${orderId}&name=${encodeURIComponent(state.user?.name || 'Student')}&email=${encodeURIComponent(state.user?.email || '')}&plan=${planId}`;
  window.open(paymentUrl, '_blank');

  showToast('💳 Payment page opening... Complete payment there.');

  // Poll for payment confirmation automatically
