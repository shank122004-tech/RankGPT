...
(function () {
  ...
  function checkPendingOnLoad() {
    try {
      ...
      verifyOrder(p.orderId).then(result => {
        if (result?.status === 'PAID') {
          const pid = p.planId || ''; if (pid === 'companion_bf_addon' || (pid.includes('boyfriend') && pid.includes('yearly'))) { pid.includes('yearly') ? activateCompanionYearly('boyfriend') : activateCompanion('boyfriend'); }
          else if (pid === 'companion_gf_addon' || (pid.includes('girlfriend') && pid.includes('yearly'))) { pid.includes('yearly') ? activateCompanionYearly('girlfriend') : activateCompanion('girlfriend'); }
          else if (p.isAddon) { activateAddon(pid); } else { activatePlan(pid || 'premium'); }
          localStorage.removeItem('crackai_pending_pay');
        }
      });
    } catch {}
  ...
  window.renderPremiumModal = function () {
    ...
    modal.innerHTML = `
      <div class="pf-header">
        <div class="pf-badge">⭐ Premium Plan</div>
        <h2 class="pf-title">Unlock Full Access</h2>
        <p class="pf-sub">Everything unlimited — one simple plan</p>
        <div class="pf-trust">
      ...
      <div class="pf-cards pf-cards-single">
        ${Object.values(PLANS).map(plan => {
          const isActive = isPrem && curPlan === plan.id;
          return `
          <div class="pf-card pf-card-popular pf-card-solo ${isActive ? 'pf-card-active' : ''}">
            <div class="pf-popular-tag">🏆 Best Value</div>
            ...
            <button class="pf-buy-btn ${isActive ? 'pf-btn-active' : ''}"
                    ...
                    ${isActive ? 'disabled' : ''}>
              ${isActive ? '✅ Active' : `💳 Buy – ₹${plan.price}/month`}
            </button>
          </div>`;
        }).join('')}
      </div>
      ...
      <div class="pf-features">
        <div class="pf-feat"><span>✅</span><span>Unlimited text questions</span></div>
        <div class="pf-feat"><span>✅</span><span>Unlimited image solving</span></div>
        <div class="pf-feat"><span>✅</span><span>Unlimited PDF solving</span></div>
      ...
      <div class="pf-companion-offer-banner">🔥 Special Launch Offer — <strong style="color:#FF6B9D;">₹49/month</strong> or <strong style="color:#7C72FF;">₹499/year</strong></div>
      ...
      <div class="pf-companion-cards">
        ${(['boyfriend','girlfriend']).map(persona => {
          ...
          <div class="pf-companion-card ${isBF ? 'pf-companion-bf' : 'pf-companion-gf'}">
            ...
            <div class="pf-companion-emoji" style="display:flex;align-items:center;justify-content:center;height:90px;"><canvas id="pf-3d-avatar-${persona}" width="90" height="90" style="border-radius:50%;display:block;"></canvas></div>
    ...
    setTimeout(() => {
      initCompanion3DAvatar('pf-3d-avatar-boyfriend', 'boyfriend');
      initCompanion3DAvatar('pf-3d-avatar-girlfriend', 'girlfriend');
    }, 80);
  };
