...
(function () {
  ...
  function patchPersona() {
    var _origSelect = window.selectPersona;
    window.selectPersona = function (persona) {
      var locked = ['boyfriend', 'girlfriend'];
      if (locked.indexOf(persona) !== -1 && !isPremium()) {
        try { if (typeof showToast === 'function') showToast('🔒 Companion personas require Premium — ₹199/month'); } catch(ex){}
        openPremium();
        return;
      }
      if (typeof _origSelect === 'function') return _origSelect.apply(this, arguments);
    };
    window.handlePersonaSettingsChange = function (selectEl) {
      var persona = selectEl.value;
      var locked = ['boyfriend', 'girlfriend'];
      if (locked.indexOf(persona) !== -1 && !isPremium()) {
        var prev = (typeof state !== 'undefined' && state.aiPersona) || '';
        selectEl.value = prev;
        try { if (typeof showToast === 'function') showToast('🔒 Companion personas require Premium — ₹199/month'); } catch(ex){}
        openPremium();
        return;
      }
      if (typeof _origSelect === 'function') _origSelect(persona);
    };
  }
  patchPersona();
