/* ============================================================
   ...
   Models:
     ...
     teacher    → Speak → STT → DeepSeek → Google TTS (Premium ₹499)
*/
...
const VOICE_PLAN_PRICE = 499;
const VOICE_PLAN_NAME  = 'Teacher Mode Pro';
...
const MODELS = {
  ...
  teacher:    { icon: '👩‍🏫', label: 'Teacher Mode',    chipName: 'Teacher',     type: 'voice',   chipClass: 'teacher-chip' },
};
...
function _initModelSelector() {
  ...
  drop.addEventListener('click', (e) => {
    ...
    // Teacher premium gate
    if (modelId === 'teacher' && !_isTeacherPremium()) {
      _closeAllDropdowns();
      openTeacherPaywall();
      return;
    }
...
function selectModel(modelId) {
  ...
  if (container) {
    ...
    if (m.type === 'voice') {
      container.classList.add(modelId === 'teacher' ? 'teacher-mode-active' : 'voice-mode-active');
...
.vai-lang-btn.active { border-color: #6C63FF; background: rgba(108,99,255,0.25); color: #fff; }
...
.tpw-title { font-family:'Space Grotesk',sans-serif; font-size:22px; font-weight:700; color:#fff; margin-bottom:8px; }
...
.tpw-features li { font-size:13px; color:rgba(255,255,255,0.8); display:flex; align-items:center; gap:8px; }
.tpw-features li::before { content:'✓'; color:#FF6B9D; font-weight:700; font-size:14px; flex-shrink:0; }
.tpw-price { font-family:'Space Grotesk',sans-serif; font-size:32px; font-weight:700; color:#fff; margin-bottom:4px; }
...
function _buildVoiceDOM() {
  ...
  paywall.innerHTML = `
    <div class="tpw-card">
      ...
      <div class="tpw-price">₹499 <span>/ month</span></div>
...
async function _handleVoiceTranscript(transcript) {
  ...
  let aiResponse = '';
  try {
    const extraPrompt = isVoice
      ? '\n\n[IMPORTANT: This answer will be read aloud. Use natural spoken sentences. No markdown, bullets, asterisks. Keep under 140 words. '
        + (isTeacher ? 'Use warm, encouraging teacher tone with storytelling.]' : 'Be conversational and clear.]')
      : '';

    if (typeof callDeepSeek === 'function') {
      const voiceMsg = transcript + (extraPrompt ? ' ' + extraPrompt : '');
      aiResponse = await callDeepSeek(voiceMsg, typeof currentMessages !== 'undefined' ? currentMessages.slice(-6) : []);
    } else {
...
async function _speakResponse(text) {
  ...
  if (voiceState.model === 'teacher' && window.GOOGLE_TTS_KEY) {
...
async function _speakBrowserTTS(text) {
  return new Promise((resolve) => {
    ...
    const preferred = voices.find(v => v.lang === 'en-IN' || v.lang === 'hi-IN' || v.name.includes('Google') || v.name.includes('India'))
      || voices.find(v => v.lang.startsWith('en')) || voices[0];
    if (preferred) utter.voice = preferred;
    utter.lang  = voiceState.recognitionLang.startsWith('hi') ? 'hi-IN' : 'en-IN';
    utter.rate  = voiceState.speechRate;
    utter.pitch = voiceState.speechPitch;
    utter.volume = 1;
...
function _observeMessages() {
  ...
  messages.addEventListener('click', (e) => {
    ...
    stopSpeaking();
    document.querySelectorAll('.msg-speak-btn.speaking').forEach(b => { b.classList.remove('speaking'); b.textContent = '🔊'; });
    btn.classList.add('speaking');
    btn.textContent = '⏹';
    const text = _stripMarkdown(bubble.innerText || bubble.textContent);
    (async () => {
      await _speakBrowserTTS(text);
      btn.classList.remove('speaking');
      btn.textContent = '🔊';
    })();
  });

  // Add speak btn to new AI messages
  const observer = new MutationObserver((mutations) => {
    mutations.forEach(m => {
      m.addedNodes.forEach(node => {
        if (node.nodeType !== 1 || !node.classList?.contains('ai')) return;
        const actions = node.querySelector('.msg-actions');
        if (!actions || actions.querySelector('.msg-speak-btn')) return;
...
function _addVoiceSettings() {
  ...
  section.innerHTML = `
    <div style="font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:600;color:var(--text-secondary);margin-bottom:12px;letter-spacing:0.04em;text-transform:uppercase;">🔊 Voice Settings</div>
    <div style="display:flex;flex-direction:column;gap:12px;">
      <div>
        <label style="font-size:12px;color:var(--text-muted);display:flex;justify-content:space-between;">Speech Speed <span id="speechRateVal">1.0×</span></label>
        <input type="range" id="speechRateSlider" min="0.5" max="2.0" step="0.1" value="1.0" style="width:100%;margin-top:4px;accent-color:#6C63FF;" />
      </div>
      <div>
        <label style="font-size:12px;color:var(--text-muted);display:flex;justify-content:space-between;">Voice Pitch <span id="speechPitchVal">1.0</span></label>
        <input type="range" id="speechPitchSlider" min="0.5" max="1.5" step="0.05" value="1.0" style="width:100%;margin-top:4px;accent-color:#FF6B9D;" />
      </div>
      <div style="font-size:11px;color:var(--text-muted);">Teacher Mode uses Google TTS when key is configured. All other voice modes use browser TTS.</div>
    </div>
  `;
